...
def get_persons_by_rollnumber(rollnumber: int, fathers_name: str, db: Session = Depends(get_db)):
    return service.get_persons_by_rollnumber(db, rollnumber, fathers_name)

@router.post('/persons/')
def create_persons(rollnumber: str, fullname: str, age: str, profession: str, virtual_age: int, db: Session = Depends(get_db)):
    return service.create_persons(db, rollnumber, fullname, age, profession, virtual_age)
